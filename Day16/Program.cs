...
namespace Day16
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("result = {0}, len = {1}",result, lineLen);
            stopwatch.Stop();
            Console.WriteLine("Elapsed Time is {0} ms", stopwatch.ElapsedMilliseconds);
        }

        private static int processInput(int pracIndex)
        {
            ...
            pracIndex += 3;
            if (typeID == 4)
                pracIndex = processLiteral(pracIndex);
            else
                pracIndex = processOperator(pracIndex);

            return pracIndex;
        }

        private static int processOperator(int pracIndex)
        {
            ...
            if (lenType == 0)
            {
                ...
                while (pracIndex < finalniIndex)
                    pracIndex = processInput(pracIndex);
            }
            else
            {
        ...
        private static int processLiteral(int pracIndex)
        {
